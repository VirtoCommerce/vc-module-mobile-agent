...
namespace VirtoCommerce.Mobile.ApiClient
{
    public static class HmacUtility
    {
        ...
        public static string ComputeHash(string secretKey, string data)
        {
        ...
        public static string ComputeHash(byte[] secretKey, byte[] data)
        {
            if (data == null)
            {
            ...
            using (CryptographicHash hasher = algorithm.CreateHash(secretKey))
            {

                hasher.Append(data);
                byte[] mac = hasher.GetValueAndReset();
                return ConvertBytesToHexString(mac);
            }
        }
